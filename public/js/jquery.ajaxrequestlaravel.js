...
(function( $ ) {
   ...
   $.fn.ajaxRequestLaravel = function(parameters) {
      ...
      var loader = (params.show_result_loader==true) ? 
      	"<div style='padding:150px' class='col-lg-12'><center><div class='la-line-scale la-dark mt-0 mb-0'><div></div><div></div><div></div><div></div><div></div></div></center></div>" : "...";

      function showResult(url_get, show_to, modal=null){
        	if (modal!==null) { $('#'+modal).modal('hide'); }

	       	$('#'+show_to).html('').append(loader).fadeTo(200,1,function(){
	            $.ajax({
	               type: "GET",
	               url: url_get,
	               cache: false,
	      	...
	      	return this.each( function() {
	        	...
	        	if (params.form_id/* && params.action_url*/) {
	               ...
	               // console.log(input_name);
					...
					$('#'+params.btn_submit_id).html(params.btn_submit_text+'&nbsp;<img style="width:20px" src="/storage/ajax-loader/loading4.gif"/>').prop('disabled', true).fadeTo(200, 1, function(){
						...
						$.ajax({
				         ...
				         error: function(result){
			            	...
			            	if(result.status === 422) {
				            	...
				            	if (params.msgbox_id) {
					            	$('#'+params.msgbox_id).show().html('<center>Oops! Something went wrong. Please try again.</center>').addClass('alert alert-danger');
					            }
				            }
