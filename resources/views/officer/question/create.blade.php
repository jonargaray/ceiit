...
@section ('body')
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-10">
            <h1>
                <a href="{{ route('officer.questions.index', [$request->prog_language_id, $request->question_set_id]) }}"><i class="fa fa-fw fa-arrow-left text-black"></i></a>
                New Question</h1>
            <ol class="breadcrumb">
                ...
                <div class="col-lg-12">
                    ...
                    <div class="ibox ">
                        ...
                        <div class="ibox-content">
                            <form method="post" action="{{ route('officer.questions.store') }}" onsubmit="btnSubmit.disabled = true">
                                @csrf
                                @method('POST')
                                <input type="hidden" name="prog_language_id" value="{{$request->prog_language_id}}">
                                <input type="hidden" name="question_type_id" value="{{$request->question_type_id}}">
                                <input type="hidden" name="question_set_id" value="{{$request->question_set_id}}">
                                ...
                                <div class="form-group  row"><label class="col-sm-2 col-form-label">Question  #{{ $numbering }} <span class="text-danger">*</span></label>
                                    <div class="col-sm-10"> 
                                        <textarea name="question" autofocus rows=5 block class="summernote form-control w-100" required></textarea>
                                    </div>
                                </div>
                                ...
                                <div class="form-group  row">
                                    <label class="col-sm-2 col-form-label">
                                        <input type="radio" required value="{{$i}}" name="correct_answer" class="mx-2 pull-right">
                                    </label>
                                    <div class="col-sm-10">
                                        <textarea name="choices[]" rows=2 block class="form-control w-100"></textarea>
                                    </div>
                                </div>
                                @endfor
